...
@interface WTReadView ()
@property (nonatomic,strong) WTMagnifierView *magnifierView;
@end
@implementation WTReadView
{
...
{
    if (!_magnifierView) {
        self.magnifierView = [[WTMagnifierView alloc] init];
...
-(void)longPress:(UILongPressGestureRecognizer *)longPress
{
    ...
    if (longPress.state == UIGestureRecognizerStateBegan || longPress.state == UIGestureRecognizerStateChanged) {
        CGRect rect = [WTReadParser parserRectWithPoint:point range:&_selectRange frameRef:_frameRef];
        [self showMagnifier];
...
-(void)pan:(UIPanGestureRecognizer *)pan
{
    ...
    if (pan.state == UIGestureRecognizerStateBegan || pan.state == UIGestureRecognizerStateChanged) {
        ...
        if (_selectState) {
            ...
            NSArray *path = [WTReadParser parserRectsWithPoint:point range:&_selectRange frameRef:_frameRef paths:_pathArray direction:_direction];
            _pathArray = path;
            [self setNeedsDisplay];
        }
...
-(void)menuCopy:(id)sender
{
    ...
    [WTReadUtilites showAlertTitle:@"成功复制以下内容" content:pasteboard.string];
...
-(void)menuNote:(id)sender
{
    ...
    UIAlertAction *confirm = [UIAlertAction actionWithTitle:@"确认" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        WTNoteModel *model = [[WTNoteModel alloc] init];
        model.content = [_content substringWithRange:_selectRange];
        model.note = alertController.textFields.firstObject.text;
        model.date = [NSDate date];
        [[NSNotificationCenter defaultCenter] postNotificationName:WTNoteNotification object:model];
    }];
