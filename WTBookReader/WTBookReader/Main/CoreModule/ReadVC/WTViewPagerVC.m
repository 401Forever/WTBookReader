...
@interface WTViewPagerVC ()<UIPageViewControllerDelegate,UIPageViewControllerDataSource>
{
    ...
    WTViewPagerTitleButton *oldButton;
...
-(void)setDataSource:(id<WTViewPagerVCDataSource>)dataSource
{
    ...
    if ([self.dataSource respondsToSelector:@selector(numberOfViewControllersInViewPager:)]) {
        ...
        for (int i = 0; i<numberOfViewController; i++) {
            ...
            if ([self.dataSource respondsToSelector:@selector(viewPager:titleWithIndexOfViewControllers:)]) {
                ...
                {
                    button = [[WTViewPagerTitleButton alloc] init];
                }
        ...
        if (mutableArrayOfBtn.count && ((UIButton *)mutableArrayOfBtn.lastObject).frame.origin.x + ((UIButton *)mutableArrayOfBtn.lastObject).frame.size.width<self.view.frame.size.width) //当所有按钮尺寸小于屏幕宽度的时候要重新布局
        {
            ...
            for (WTViewPagerTitleButton *button in mutableArrayOfBtn) {
...
-(void)p_titleButtonClick:(WTViewPagerTitleButton *)sender
{
...
-(void)p_titleButtonConvert:(WTViewPagerTitleButton *)sender
{
    oldButton.selected = NO;
    sender.selected = YES;
    oldButton = sender;
...
@implementation WTViewPagerTitleButton
- (instancetype)init
{
    self = [super init];
    if (self) {
